    ...
    <section className='bg-gradient-to-t from-zinc-50 to-white dark:from-zinc-950'>
      <div className='mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-24 lg:px-8'>
        <div className='mx-auto max-w-4xl'>
          ...
          <Accordion key={item.title} type='single' collapsible className='space-y-4 mt-8'>
              <AccordionItem  value={`item-${idx}`} className='rounded-lg shadow-sm '>
                <AccordionTrigger className='px-4 py-4'>
                  <span className='text-left font-medium'>{item.title}</span>
                </AccordionTrigger>
                <AccordionContent className='px-4 pb-4 pt-2 text-gray-300'>{item.description}</AccordionContent>
              </AccordionItem>
          </Accordion>
           ))}
        </div>
