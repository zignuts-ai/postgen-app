    ...
    <header className='sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60'>
      <div className='max-w-7xl mx-auto flex h-14 items-center px-4 sm:px-6 lg:px-8'>
        <div className='mr-4 hidden md:flex'>
          ...
          <Link href={header.brand?.url || ''} className='text-lg font-medium mr-16'>
            {header.brand && header.brand.avatar ? (
              <div className='flex items-center gap-x-2 cursor-pointer'>
                <img src={header.brand.avatar.src} alt={header.brand.avatar.title} className='h-8' />
                <h1 className='text-xl font-bold'>{header.brand.title}</h1>
              </div>
            ) : (
              <h1 className='text-xl font-bold cursor-pointer'>{header.brand?.title}</h1>
            )}
          </Link>
          ...
          <nav className='flex items-center space-x-6 text-sm'>
            {header.nav?.items?.map((v: Item, idx: number) => {
              return (
                <Link href={v.url || ''} target={v.target || '_self'} key={idx}>
                  {v.title}
                </Link>
              )
            })}
